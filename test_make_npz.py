...
dst_dir = Path("libraries/avatars/Xiangling/outputs")
...
P = V_dst_for_matching

head_pos = rest_joints[HEAD_JOINT]
neck_pos = rest_joints[NECK_JOINT]
head_neck_len = np.linalg.norm(head_pos - neck_pos)

dist_to_head = np.linalg.norm(P - head_pos[None, :], axis=1)

head_radius = max(
    2.8 * head_neck_len,
    0.10 * np.max(V_src.max(axis=0) - V_src.min(axis=0)),
)

upper_head_threshold = h_min + 0.88 * body_h
lower_head_threshold = h_min + 0.80 * body_h

upper_head_mask = (h > upper_head_threshold) & (dist_to_head < head_radius)
lower_head_mask = (
    (h > lower_head_threshold)
    & (h <= upper_head_threshold)
    & (dist_to_head < 1.25 * head_radius)
)

# Do not let arm/sleeve-looking vertices be stolen by the head fix.
arm_joints = [
...
arm_score = W_dst[:, arm_joints].sum(axis=1)
arm_like_mask = arm_score > 0.35

upper_head_mask = upper_head_mask & ~arm_like_mask
lower_head_mask = lower_head_mask & ~arm_like_mask
...
print("excluded arm-like vertices:", np.count_nonzero(arm_like_mask))
